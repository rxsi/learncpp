...
流程:
1. 在当父进程创建管道,
2. 再通过fork的方式创建子进程,子进程会继承复制管道的文件描述符
...
1. 读端不读,写端一直写,读端不关闭fd[0]:
    当内核缓冲区被写满之后,会阻塞写端的write函数

2. 写端不写,读端一直读,写端不关闭fd[1]:
    当内核缓冲区数据被读完之后,会阻塞读端的read函数
...
//             // size_t一般用来表示"适用于计量neiucn中可容纳的数据项目个数的无符号整数类型",因此通常用在数组下标和内存管理函数之类的函数
