    ...
    while (true){
        ...
        for (size_t i = 0; i < n; ++i){
           if (epoll_events[i].events & EPOLLIN){ // 发生的事件是可读事件
                ...
                if (epoll_events[i].data.fd != listenfd){
                    std::cout << "EPOLLOUT triggered, clientfd: " << epoll_events[i].data.fd << std::endl; // 采用lt模式时,会一直触发,可以在每次触发过后,修改为不见听EPOLLOUT事件,等要监听时再注册
                }
...
/*
当多线程同时处理一个socket时,可能会出现这种情况:A线程正在处理,刚读取到一半,由于时间片到了,因此切换到B线程继续处理,这样会造成异常.在LT模式和ET模式都会
处理方式有:
1. 规定只能单个线程读取
2. 加锁
3. 使用EPOLLONESHOT

EPOLLONESHOT: 限定对应事件只能被调用一次,这和ET模式的区别在于,ET触发后如果唤醒了多个线程(惊群现象),那么这些线程都有权处理该fd事件.
