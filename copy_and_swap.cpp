...
//         cout << "Base(int size_num) = " << size_num << endl;
//         ptr = new int[size];
//     }

//     Base(const Base& base): size(base.size), ptr(new int[size]){ //可以通过调用构造函数的方式,减少拷贝构造函数的代码冗余
...
//     Base& operator=(Base base){ // 版本2:先确保能够成功创建新数据,再释放旧数据,这样确保了安全性
...
// {
//     std::cout << "swap(Base& a, Base& b)" << endl;
//     using std::swap;
//     swap(a.size, b.size);
//     swap(a.ptr, b.ptr); 
// }
...
// void fun(T& b)
// {
//     std::cout << "tempalte fun()" << std::endl;
// }
